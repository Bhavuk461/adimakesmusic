...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    // A shader the GPU rejects must not throw: an error escaping an effect
    // unmounts the whole React tree. Bail out and leave the canvas blank.
    const compile = (type: number, src: string) => {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        console.warn("SiriWave: shader failed to compile", gl.getShaderInfoLog(shader))
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.warn("SiriWave: program failed to link", gl.getProgramInfoLog(program))
